 ...
 * @author santi
 */
public class PlayerActionGenerator_SCV {
    ...
    public PlayerActionGenerator_SCV(GameState a_gs, int pID, HashSet<Unit> unitsAbsAB, PlayerAction baseActions) throws Exception {
        this.baseAction = baseActions;
            ...
            if (u.getPlayer() == pID) {
                if (gs.unitActions.get(u) == null) {
                    ...
                    if (unitIsControlled(u)) {
                        ...
                        //se a unidade não for controlada obtenho a ação do baseActions
                        UnitAction tUnit = baseAction.getAction(u);
                        if (tUnit == null) {
                            l = u.getUnitActions(gs);
                        } else {
                            l.add(tUnit);
...
//        System.out.println("---");
    ...
    public PlayerActionGenerator_SCV(GameState a_gs, int pID, AI Enemy) throws Exception {
        this.enemy = Enemy;
        // Generate the reserved resources:
        base_ru = new ResourceUsage();
        gs = a_gs;
        ...
        for (Unit u : pgs.getUnits()) {
            if (u.getPlayer() == pID) {
                if (gs.unitActions.get(u) == null) {
                    //List<UnitAction> l = u.getUnitActions(gs);
                    enemy.reset();
                    PlayerAction actEnemy = enemy.getAction(pID, gs);
                    List<UnitAction> l = takeUnitScriptMove(u, gs, pID);;
